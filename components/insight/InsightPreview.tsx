...
export function InsightPreview({ insight, onClose, onNavigateDate }: InsightPreviewProps) {
  ...
  return (
    <div className="space-y-4 sm:space-y-5 relative">
      ...
      {/* Insight Text - 카드 스타일 */}
      <div className="relative py-5 px-4 sm:py-4 sm:px-2">
        {/* 배경 장식 */}
        ...
        <div className="relative">
          <h3 className="text-body font-bold leading-relaxed text-center text-label">
      ...
      {/* Keywords - 태그 스타일 */}
      <div className="flex flex-wrap justify-center gap-2 px-4 sm:px-2">
        {insight.keywords.map((kw) => (
          <span 
            key={kw.keyword}
            className="px-2.5 py-1 rounded-full bg-bg-secondary text-footnote font-medium text-label/60 sm:text-label/55"
          >
            {kw.keyword}
          </span>
        ))}
